 ...
 *
 */
public class DiagnosisEditor extends JFrame implements ActionListener{
	...
	private JList list;
	private JRadioButton rdbtnDiagnosis;
	...
	public DiagnosisEditor(int nSimPort, String sSessId, String sProcId, boolean bDebug) {
		setTitle("Diagnosis Editor");
		...
		syscaller = new SysCallerImpl(nSimPort, sSessId, sProcId, bDebug, "Diagnosis Editor");
		...
		panel_1.setBorder(BorderFactory.createCompoundBorder(BorderFactory.createEmptyBorder(10, 0, 0, 0),
				BorderFactory.createTitledBorder("Diagnosis:")));
		...
		btnSubmit = new JButton(getSubmitToDiagnosis());
		...
		rdbtnDiagnosis = new JRadioButton("Diagnoses");
		sl_panel_2.putConstraint(SpringLayout.WEST, scrollPane, 0, SpringLayout.WEST, rdbtnDiagnosis);
		sl_panel_2.putConstraint(SpringLayout.WEST, rdbtnDiagnosis, 10, SpringLayout.WEST, panel_2);
		sl_panel_2.putConstraint(SpringLayout.NORTH, rdbtnDiagnosis, 0, SpringLayout.NORTH, panel_2);
		panel_2.add(rdbtnDiagnosis);

		rdbtnDrafts = new JRadioButton("Drafts");
		sl_panel_2.putConstraint(SpringLayout.WEST, rdbtnDrafts, 10, SpringLayout.WEST, panel_2);
		sl_panel_2.putConstraint(SpringLayout.NORTH, rdbtnDrafts, 6, SpringLayout.SOUTH, rdbtnDiagnosis);
		...
		sl_panel_2.putConstraint(SpringLayout.NORTH, lblFrom, 4, SpringLayout.NORTH, rdbtnDiagnosis);
		panel_2.add(lblFrom);
		...
		sl_panel_2.putConstraint(SpringLayout.NORTH, textFromMonth, 1, SpringLayout.NORTH, rdbtnDiagnosis);
		...
		sl_panel_2.putConstraint(SpringLayout.NORTH, lblTo, 4, SpringLayout.NORTH, rdbtnDiagnosis);
		panel_2.add(lblTo);

		textToMonth = new JTextField();
		sl_panel_2.putConstraint(SpringLayout.EAST, lblTo, -6, SpringLayout.WEST, textToMonth);
		sl_panel_2.putConstraint(SpringLayout.NORTH, textToMonth, 1, SpringLayout.NORTH, rdbtnDiagnosis);
		...
		sl_panel_2.putConstraint(SpringLayout.NORTH, textFromDay, 1, SpringLayout.NORTH, rdbtnDiagnosis);
		...
		sl_panel_2.putConstraint(SpringLayout.NORTH, textToDay, 1, SpringLayout.NORTH, rdbtnDiagnosis);
		...
		sl_panel_2.putConstraint(SpringLayout.NORTH, textToYear, 1, SpringLayout.NORTH, rdbtnDiagnosis);
		...
		sl_panel_2.putConstraint(SpringLayout.NORTH, textFromYear, 1, SpringLayout.NORTH, rdbtnDiagnosis);
		...
		btnClearSearch.addActionListener(new ActionListener(){
			...
			public void actionPerformed(ActionEvent arg0) {
				...
				rdbtnDiagnosis.setSelected(false);
		...
		JButton btnViewAllTreatments = new JButton("View All Diagnoses");
		btnViewAllTreatments.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				String id1 = syscaller.getIdOfEntityWithNameAndType("Diagnoses", "b");
				String[] results;
				List<String> newResults = new ArrayList<String>();
				String sType = "b";
				String sGraphType = "ac";
				List<String[]> members = syscaller.getMembersOf("Diagnoses", id1, sType, sGraphType);
				...
				for(int i = 0; i < newResults.size(); i++){
					...
					String tn = props.getProperty("dn");
					...
					System.out.println("DIAGNOSIS    " + treat);
	...
	public void openAll() {
		...
		if(rdbtnDiagnosis.isSelected()){
			reader.setTitle("Diagnoses");
			for(int j = 0; j < found.size(); j++){
				...
				String diagnosis = "Date:\t" + date
						...
						+  "\nDiagnosis:\t" + fDiag
						+ "\n=============================================================\n";			
				reader.write(diagnosis);
			}
			reader.setVisible(true);
		}else if(rdbtnDrafts.isSelected()){
			reader.setTitle("Diagnoses Drafts");
			for(int j = 0; j < found.size(); j++){
	...
	public static void createGUI(){
		DiagnosisEditor draft = new DiagnosisEditor(simport, sessid, pid, debug);
		draft.pack();
	...
	class SaveDraftAlt extends AbstractAction {
		...
		public void actionPerformed(ActionEvent e){
			...
			String sContainers = "b|PatDiagDrafts,b|DraftDiagnoses";
			...
			if (mrnField.getText().length() == 0) {
				...
				if(sdiag.length() == 0){
					JOptionPane.showMessageDialog(DiagnosisEditor.this, "Fill Diagnosis field");
					return;
				}
				...
				if (sHandle == null) {
					JOptionPane.showMessageDialog(DiagnosisEditor.this,
				...
				props.put("dn", sObjName);
				props.put("ssn", sssn);
				props.put("name", slast);
				props.put("diagnosis", sdiag);
	...
	//////////////////////////////////////SUBMIT///////////////////////////////////////////////////
	public Action getSubmitToDiagnosis(){
		return submitDiag;
	}

	private Action submitDiag = new SubmitToDiagnosis();

	class SubmitToDiagnosis extends AbstractAction{
		public SubmitToDiagnosis(){
			super("Submit to Diagnosis");
		...
		public void actionPerformed(ActionEvent e){
			...
			//If this diagnosis has not been saved to a draft before
			if(sObjName.length() == 0){
				...
				String sContainers = "b|PatDiag,b|Diagnoses";
				...
				if(sHandle == null){
					JOptionPane.showMessageDialog(DiagnosisEditor.this, syscaller.getLastError());
					System.out.println("sHandle was null");
					return;
				}
				...
				//if the diagnosis has been saved as a draft and already has a diagnoses number	
				...
				String sProp1 = "PatDiagDrafts";
				String sContainer1 = "PatDiag";
				System.out.println(sContainer1 + " " + sProp1);
				String sContainer2 = "Diagnoses";
				String sProp2 = "DraftDiagnoses";
				...
				if(!cmd){
					System.out.println("DELETEASSIGNEMNT didn't work 1");
					return;
				}*/
				...
				String sHandle = syscaller.createObject3(sObjName, "File", "rtf",
						"b|PatDiag,b|Diagnoses", "File read", null, null, null, null);
			...
			btnSubmit.setEnabled(false);
			JOptionPane.showMessageDialog(DiagnosisEditor.this, "Submit Sucessful!");
			btnSave.setEnabled(false);
	...
	class SearchAction extends AbstractAction{
		...
		public void search(){
			...
			if(rdbtnDiagnosis.isSelected() && rdbtnDrafts.isSelected()){
				JOptionPane.showMessageDialog(DiagnosisEditor.this, "Only select one");
				return;
			}else if(!rdbtnDiagnosis.isSelected() && !rdbtnDrafts.isSelected()){
				JOptionPane.showMessageDialog(DiagnosisEditor.this, "Please select Drafts or Diagnoses");
				return;
			}

			if(sel.length() == 0){
				JOptionPane.showMessageDialog(DiagnosisEditor.this, "Please select one");
				return;

			}
			if(sel.equals("diagnoses")){
				String id1 = syscaller.getIdOfEntityWithNameAndType("Diagnoses", "b");

				String sType = "b";
				String sGraphType = "ac";
				List<String[]> members = syscaller.getMembersOf("Diagnoses", id1, sType, sGraphType);
				System.out.println("before getting Diagnoses");
				for(int i = 0; i < members.size(); i++){
					results = (String[])members.get(i);
					System.out.println("member " + i + ":" + results[0] + ":" + results[1] + ":" + results[2]);
					if(results[0].equals("o") && results[2].length() == 8){
				...
				String id1 = syscaller.getIdOfEntityWithNameAndType("DraftDiagnoses", "b");

				String sType = "b";
				String sGraphType = "ac";
				List<String[]> members = syscaller.getMembersOf("DraftDiagnoses", id1, sType, sGraphType);
			...
			try{
				...
				for(int i = 0; i < newResults.size(); i++){
					...
					if(patient.equals(sLast)){//firstField.getText() + " " + midField.getText() + " " + lastField.getText())){
						...
						String diagDate =  month + "/" + day + "/" + year;
						System.out.println("Diagnosis date: " + diagDate);
						...
						Date dDate = format.parse(diagDate);
	...
	class OpenAction extends AbstractAction{	
		...
		public void select(){
			...
			if(nSelIx < 0){
				JOptionPane.showMessageDialog(DiagnosisEditor.this, "Select a result");
				return;
			}
			...
			mrnField.setText(props.getProperty("dn"));
			...
			myEditorPane.setText(props.getProperty("diagnosis"));
